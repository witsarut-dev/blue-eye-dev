...
// through culture var or from URL
    if ($.urlParam('langCode') != 0 && file_exists('scripts/languages/' + $.urlParam('langCode') + '.js'))
...
// 'cap' is one of 'select', 'rename', 'delete', 'download', move
    function has_capability(data, cap) {
        if (data['File Type'] == 'dir' && (cap == 'download' || cap == 'replace'))
            return false;
...
// Test if file is authorized
    var isAuthorizedFile = function(filename) {
        if (config.security.uploadPolicy == 'DISALLOW_ALL') {
            if ($.inArray(getExtension(filename), config.security.uploadRestrictions) != -1)
                return true;
        }
...
// clicked in the file tree or list views.
    var getFileInfo = function(file) {
        ...
        if ($.inArray('select', capabilities) != -1 && (window.opener || window.tinyMCEPopup || $.urlParam('field_name')))
