...
class Realtime extends Frontend {
	...
	function view()
	{
		...
		$viewdata['competitor'] = $this->realtime_model->get_competitor();
	...
	function get_is_read(){
		...
		$post['period'] = $period;
		$result = array();
		$result['data'] = $this->realtime_model->get_is_read($post);
	...
	function update_is_read(){
		...
		$this->realtime_model->update_is_read($post);
		// $this->master_model->save_log("Update Msg ID " . $post_id);
	...
	function get_feed()
	{
		...
		if ($post["company_type"]) {
			if ($post["company_type"] == "client") {
				$viewdata['rowsdata'] = $this->realtime_model->get_feed($post, "own");
			} else if ($post["company_type"] == "competitor") {
				$viewdata['rowsdata'] = $this->realtime_model->get_feed($post, "competitor");
			} else {
	...
	function add_feed()
	{
		...
		if (isset($post["last_time"])) {
			...
			if ($post["company_type"]) {
				if ($post["company_type"] == "client") {
					$viewdata['rowsdata'] = $this->realtime_model->add_feed($post, "own");
				} else if ($post["company_type"] == "competitor") {
					$viewdata['rowsdata'] = $this->realtime_model->add_feed($post, "competitor");
				} else {
					$viewdata['rowsdata'] = $this->realtime_model->add_feed($post);
				}
			}
