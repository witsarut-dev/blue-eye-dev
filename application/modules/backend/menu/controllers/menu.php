...
class Menu extends Backend 
{
    ...
    function __construct()
    {
        ...
        parent::$view_data["order_mode"] = "ON";
	...
	function formList()
	{
		...
		$rows = $this->menu_model->get_rows($post);
		parent::$view_data["pagesize"] = 15;
		parent::$view_data["rows"] = $rows;
		parent::$view_data["rows_publish"] = $this->menu_model->get_rows_publish($post);
		parent::$view_data["rows_modified"] = $this->menu_model->get_rows_modified($post);
		parent::$view_data["rows_unpublish"] = $this->menu_model->get_rows_unpublish($post);
		parent::$view_data["totalpage"] = ceil($rows/parent::$view_data["pagesize"]);
		parent::$view_data["post"] = $post;
		parent::$view_data["control"] = true;

		$this->load->view("menu_list_view",parent::$view_data);
	}
	...
	function ajaxList()
	{
        ...
        if($post) {
    		...
    		$start = $post["thispage"];
    		$end = $post["pagesize"];
    		$item = $this->menu_model->get_page($option,$start,$end);
	...
	function formDisplay($id = 0)
	{
		...
		$this->menu_model->count_log($id,$rows,$totalpage);
		parent::$view_data["rows"] = $rows;
	...
	private function cmdSave(&$id)
	{
		...
		if($status) {
			if(empty($val)) {
				...
				$val = $this->menu_model->insert($post);
			} else {
				parent::check_access($this->module,"modified");
				$this->sys_logs_model->save_log($this->module,"modified");
				$this->menu_model->update($post);
			}
		...
		if($status) {
			...
			if($status) {
				...
				if($status) {
					...
					$this->menu_model->savePublish($val,$cmd);
	...
	private function cmdPublish(&$id)
	{
		...
		if(gettype($post["id"])=="array") {
			foreach($post["id"] as $key=>$val) {
				...
				if($status) {
					$this->sys_logs_model->save_log($this->module,"publish");
					$this->menu_model->save_public($val);
	...
	private function cmdUnPublish()
	{
		...
		foreach($post["id"] as $key=>$val) {
			...
			if($status) {
				...
				$this->menu_model->save_unpublic($val);
	...
	function cmdSaveToForm()
	{
        ...
        if($post) {
    		...
    		$site_url = site_url("menu/formEdit/".$id);
    		$result = array("status"=>$status
    			,"message"=>self::$message
    			,"url"=>$site_url);
	...
	function cmdDeleteFile()
	{
        ...
        if($post) {
    		...
    		@unlink($BASEPATH."upload/menu/thumb_edit/".@$post['filepath']);
    		@unlink($BASEPATH."upload/menu/thumb_list/".@$post['filepath']);
    		@unlink($BASEPATH."upload/menu/".@$post['filepath']);

    		$this->menu_model->delete_file($post);

    		$site_url = site_url("menu/formEdit/".@$post["id"]);
	...
	function changeLog()
	{
        ...
        if($post) {
    		...
    		$start = $post["thispage"];
    		$end = $post["pagesize"];
    		$item = $this->menu_model->get_log($option,$start,$end);
    		echo  json_encode($item);
        } else {
            show_404();
        }
	...
	function cmdUpdateParent()
	{
        ...
        if($post) {
            ...
            $this->menu_model->update_parent($json_array,0);
        } else {
            show_404();
        }
	}
	...
	function cmdUpdateStatus()
    {
        ...
        if($post) {
            ...
            parent::check_access($this->module,"modified");
            $this->menu_model->update_status($post);
        } else {
            show_404();
        }
    }
	...
	{
		$obj = modules::run('menu/menu_trigger/set_trigger', $action, $id, $data);
		self::$message = $obj["message"];
		return $obj["status"];
	}
