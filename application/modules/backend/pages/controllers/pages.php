...
class Pages extends Backend 
{
	...
	function formList()
	{
		...
		$rows = $this->pages_model->get_rows($post);
		parent::$view_data["pagesize"] = 20;
		parent::$view_data["rows"] = $rows;
		parent::$view_data["rows_publish"] = $this->pages_model->get_rows_publish($post);
		parent::$view_data["rows_modified"] = $this->pages_model->get_rows_modified($post);
		parent::$view_data["rows_unpublish"] = $this->pages_model->get_rows_unpublish($post);
		parent::$view_data["totalpage"] = ceil($rows/parent::$view_data["pagesize"]);
		parent::$view_data["post"] = $post;
		parent::$view_data["control"] = true;

		$this->load->view("pages_list_view",parent::$view_data);
	}
	...
	function ajaxList()
	{
        ...
        if($post) {
    		...
    		$start = $post["thispage"];
    		$end = $post["pagesize"];
    		$item = $this->pages_model->get_page($option,$start,$end);
	...
	function formDisplay($id = 0)
	{
		...
		$this->pages_model->count_log($id,$rows,$totalpage);
		parent::$view_data["rows"] = $rows;
	...
	private function cmdSave(&$id)
	{
		...
		if($status) {
			if(empty($val)) {
				...
				$val = $this->pages_model->insert($post);
			} else {
				parent::check_access($this->module,"modified");
				$this->sys_logs_model->save_log($this->module,"modified");
				$this->pages_model->update($post);
			}
		...
		if($status) {
			...
			if($status) {
				...
				if($status) {
					...
					$this->pages_model->savePublish($val,$cmd);
	...
	private function cmdPublish(&$id)
	{
		...
		if(gettype($post["id"])=="array") {
			foreach($post["id"] as $key=>$val) {
				...
				if($status) {
					$this->sys_logs_model->save_log($this->module,"publish");
					$this->pages_model->save_public($val);
	...
	private function cmdUnPublish()
	{
		...
		foreach($post["id"] as $key=>$val) {
			...
			if($status) {
				...
				$this->pages_model->save_unpublic($val);
	...
	function cmdSaveToForm()
	{
        ...
        if($post) {
    		...
    		$site_url = site_url("pages/formEdit/".$id);
    		$result = array("status"=>$status
    			,"message"=>self::$message
    			,"url"=>$site_url);
	...
	function cmdDeleteFile()
	{
        ...
        if($post) {
    		...
    		@unlink($BASEPATH."upload/pages/thumb_edit/".@$post['filepath']);
    		@unlink($BASEPATH."upload/pages/thumb_list/".@$post['filepath']);
    		@unlink($BASEPATH."upload/pages/".@$post['filepath']);

    		$this->pages_model->delete_file($post);

    		$site_url = site_url("pages/formEdit/".@$post["id"]);
	...
	function changeLog()
	{
        ...
        if($post) {
    		...
    		$start = $post["thispage"];
    		$end = $post["pagesize"];
    		$item = $this->pages_model->get_log($option,$start,$end);
    		echo  json_encode($item);
        } else {
            show_404();
        }
	...
	function cmdUpdateParent()
	{
        ...
        if($post) {
            ...
            $this->pages_model->update_parent($json_array,0);
        } else {
            show_404();
        }
	}
	...
	function cmdUpdateStatus()
    {
        ...
        if($post) {
            ...
            parent::check_access($this->module,"modified");
            $this->pages_model->update_status($post);
        } else {
            show_404();
        }
    }
	...
	{
		$obj = modules::run('pages/pages_trigger/set_trigger', $action, $id, $data);
		self::$message = $obj["message"];
		return $obj["status"];
	}
